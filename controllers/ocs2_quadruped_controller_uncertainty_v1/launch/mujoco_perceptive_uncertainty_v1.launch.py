...
package_controller = "ocs2_quadruped_controller_uncertainty_v1"
...
def launch_setup(context, *args, **kwargs):
    ...
    robot_controllers = os.path.join(
        ...
        "config",
        "robot_control_perceptive_uncertainty_v1.yaml",
    )
    ...
    ocs2_controller = Node(
        package="controller_manager",
        executable="spawner",
        arguments=["ocs2_quadruped_controller_uncertainty_v1", "--controller-manager", "/controller_manager"],
    )
    ...
    terrain_pipeline = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(
            os.path.join(
                ...
                "launch",
                "elevation_mapping_uncertainty_v1.launch.py",
            )
        ),
...
def generate_launch_description():
    pkg_description = DeclareLaunchArgument(
        "pkg_description",
        default_value="go2_description_uncertainty_v1",
    ...
    terrain_uncertainty_base = DeclareLaunchArgument(
        ...
        default_value="0.05",
        description="Base uncertainty layer value for uncertainty_v1 perceptive experiments.",
    )

    terrain_uncertainty_gradient_weight = DeclareLaunchArgument(
        "terrain_uncertainty_gradient_weight",
        default_value="0.35",
        description="Gradient-driven uncertainty weight for uncertainty_v1 perceptive experiments.",
    )

...
namespace ocs2::legged_robot
{
    ...
    namespace
    {
        size_t gControlInputDebugCounter = 0;
        size_t gUpdateInputSyncDebugCounter = 0;
    }
    ...
    {
        if (const auto* latest_input = control_input_buffer_.readFromRT())
        {
            ctrl_interfaces_.control_inputs_ = *latest_input;
            if ((gUpdateInputSyncDebugCounter++ % 25) == 0)
            {
                RCLCPP_INFO(
                    get_node()->get_logger(),
                    "[UpdateInputSyncDebug] t=%.3f command=%d lx=%.3f ly=%.3f rx=%.3f ry=%.3f",
                    time.seconds(),
    ...
    controller_interface::CallbackReturn Ocs2QuadrupedController::on_init()
    {
        ...
        control_input_buffer_.writeFromNonRT(control_input_msgs::msg::Inputs{});
        ...
        const rclcpp_lifecycle::State& /*previous_state*/)
    {
        control_input_subscription_ = get_node()->create_subscription<control_input_msgs::msg::Inputs>(
            ...
            {
                control_input_buffer_.writeFromNonRT(*msg);
                if ((gControlInputDebugCounter++ % 25) == 0)
                {
                    RCLCPP_INFO(
                        get_node()->get_logger(),
                        "[ControlInputDebug] command=%d lx=%.3f ly=%.3f rx=%.3f ry=%.3f",

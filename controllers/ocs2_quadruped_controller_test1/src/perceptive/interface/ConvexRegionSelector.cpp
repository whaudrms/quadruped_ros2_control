...
namespace ocs2::legged_robot
{
            ...
            for (size_t i = 0; i < numPhases; ++i)
            {
                if (contactFlagStocks[leg][i])
                {
                    ...
                    if (!numerics::almost_eq(standMiddleTime, lastStandMiddleTime))
                    {
                        ...
                        if ((debugCounter_++ % 30) == 0)
                        {
                            const auto measuredFootPositions = endEffectorKinematicsPtr_->getPosition(initState);
                            const auto desiredState = targetTrajectories.getDesiredState(standMiddleTime);
                            const auto desiredFootPositions = endEffectorKinematicsPtr_->getPosition(desiredState);
                            std::cerr << "[ConvexSelectorDebug]"
                                      << " leg=" << leg
                                      << " standMiddleTime=" << standMiddleTime
                                      << " measuredFootWorld=(" << measuredFootPositions[leg].x() << ","
                                      << measuredFootPositions[leg].y() << ","
                                      << measuredFootPositions[leg].z() << ")"
                                      << " desiredFootWorld=(" << desiredFootPositions[leg].x() << ","
                                      << desiredFootPositions[leg].y() << ","
                                      << desiredFootPositions[leg].z() << ")"
                                      << " nominalFootholdWorld=(" << footPos.x() << ","
                                      << footPos.y() << ","
                                      << footPos.z() << ")"
                                      << " projectionWorld=(" << projection.positionInWorld.x() << ","
                                      << projection.positionInWorld.y() << ","
                                      << projection.positionInWorld.z() << ")"
                                      << " projectionTerrain=(" << projection.positionInTerrainFrame.x() << ","
                                                       ...
                                                       TargetTrajectories& targetTrajectories)
    {
        ...
        nominalFoothold.x() = 0.10 * measuredFootPositions[leg].x() + 0.90 * desiredFootPositions[leg].x();
        nominalFoothold.y() = 0.05 * measuredFootPositions[leg].y() + 0.95 * desiredFootPositions[leg].y();

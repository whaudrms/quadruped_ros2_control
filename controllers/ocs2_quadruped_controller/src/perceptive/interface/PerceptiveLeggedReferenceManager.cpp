...
namespace ocs2::legged_robot
{
                                                                ...
                                                                const vector_t& initState)
    {
        ...
        // Collect (t, leg) requests in chronological order.
        ...
        // Contact detections from different legs can arrive a few control ticks
        // apart even though they belong to the same MPC shooting interval. Apply
        // such requests atomically so the schedule never contains an artificial
        // sub-dt intermediate mode (for example 6 -> 7 -> 15 in 2 ms). Use the
        // latest measured time in the batch so no contact is declared before it
        ...
        std::vector<SpliceBatch> batches;
        for (const auto& request : requests) {
            if (batches.empty() || request.first - batches.back().firstTime > dt_mpc) {
                batches.push_back({request.first, request.first, {request}});
            } else {
                batches.back().eventTime = request.first;
                batches.back().requests.push_back(request);
            }
        }
        ...
        for (const auto& batch : batches) {
            const scalar_t t = batch.eventTime;
            ...
            std::vector<std::pair<scalar_t, size_t>> activeRequests;
            for (const auto& request : batch.requests) {
                const size_t leg = request.second;
                if (!curFlags[leg]) {
                    activeRequests.push_back(request);
                    curFlags[leg] = true;
                }
            }
            if (activeRequests.empty()) continue;  // all requested legs are already in stance
            ...
            feet_array_t<scalar_t> gEvents;
            gEvents.fill(std::numeric_limits<scalar_t>::quiet_NaN());
            const auto footPositions = endEffectorKinematicsPtr_->getPosition(initState);
            {
                ...
                for (const auto& request : activeRequests) {
                    ...
                    if (w.active) {
                        gEvents[leg] = w.n.dot(footPositions.at(leg) - w.p_plane) - w.foot_frame_offset;
                    }
                }
            }

            // === Decide: insert a phase or merge with a nearby event ===
            // Merge when a request lands within dt_mpc after the preceding
            // event. This also coalesces contacts split across two MPC queue
            // drains: the second contact augments the mode created by the first
            // instead of creating a sub-dt phase. Preserve the existing
            // near-touchdown guard on the following event as well.
            bool merge = false;
            if (curPhase > 0) {
                const scalar_t previousEventTime = sched.eventTimes[curPhase - 1];
                const scalar_t timeSincePrevious = t - previousEventTime;
                if (timeSincePrevious >= 0.0 && timeSincePrevious <= dt_mpc) {
                    merge = true;
                }
            }
            ...
            feet_array_t<size_t> propagatedTo;
            propagatedTo.fill(merge ? curPhase : curPhase + 1);
            for (const auto& request : activeRequests) {
                const size_t leg = request.second;
            ...
            for (const auto& request : activeRequests) {
                const scalar_t measuredTime = request.first;
                const size_t leg = request.second;
                std::cerr << tag << " leg=" << leg << " t=" << measuredTime
                          << " splice_t=" << t
                          << " batch_size=" << activeRequests.size()
                          << " g_event=" << gEvents[leg]
                          << " (>0 high-side / <0 low-side)"
                          << " mode_old=" << oldMode
                          << " mode_new=" << newMode
                          << " propagated_phases=" << (propagatedTo[leg] - curPhase)
                          << (merge ? " (in-place; no event insert)" : "")
                          << "\n";
            }
        }

        const bool validSizes = sched.eventTimes.size() + 1 == sched.modeSequence.size();
        const bool strictlyIncreasing = std::adjacent_find(
            sched.eventTimes.begin(), sched.eventTimes.end(),
            [](scalar_t lhs, scalar_t rhs) { return lhs >= rhs; }) == sched.eventTimes.end();
        if (!validSizes || !strictlyIncreasing) {
            std::cerr << "[robust_contact_splice_rejected] invalid mode schedule after batching"
                      << " event_count=" << sched.eventTimes.size()
                      << " mode_count=" << sched.modeSequence.size() << "\n";
            return;
        }

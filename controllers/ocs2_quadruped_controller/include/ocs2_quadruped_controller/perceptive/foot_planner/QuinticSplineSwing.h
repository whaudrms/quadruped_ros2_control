// Ported from ocs2_switched_model_interface; only includes/types/namespace adapted.
//
...
namespace ocs2::legged_robot::perceptive_swing {
    struct SwingNode {
        scalar_t time;
        scalar_t position;
        scalar_t velocity;
    ...
    public:
        ...
        QuinticSpline() : c_(Eigen::Matrix<scalar_t, 6, 1>::Zero()), t0_(-1e30), dt_(2e30) {
        }
         ...
         */
        QuinticSpline(const Eigen::Matrix<scalar_t, 6, 1> &coefficients, scalar_t t0, scalar_t dt) : c_(coefficients), t0_(t0), dt_(dt) {
        }
